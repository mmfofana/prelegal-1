...
from schemas.chat import AiChatOutput, PartialNdaFields
...
class TestChatResponse:
    ...
    def test_chat_returns_fields(self, client: TestClient):
        _signup(client)
        mock_output = AiChatOutput(
            reply="Thanks!",
            fields=PartialNdaFields(purpose="evaluating a partnership", governing_law="Delaware"),
        )
        with patch("services.chat_service.get_ai_response", return_value=mock_output):
            r = client.post(_CHAT_URL, json=_VALID_PAYLOAD)
        data = r.json()
        assert data["fields"]["purpose"] == "evaluating a partnership"
        assert data["fields"]["governing_law"] == "Delaware"
        assert data["fields"]["jurisdiction"] is None

    def test_chat_passes_messages_and_fields_to_service(self, client: TestClient):
        _signup(client)
        payload = {
            "messages": [
            ...
            "current_fields": {"purpose": "Evaluating a partnership"},
        }
        ...
        messages_arg, current_fields_arg = call_args[0]
        assert len(messages_arg) == 3
        assert messages_arg[0].content == "NDA for Acme and Beta"
        assert current_fields_arg.purpose == "Evaluating a partnership"
